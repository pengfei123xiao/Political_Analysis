...
"""This file is used to manager all the analysis jobs and it is executed on a daily basis."""
    ...
    try:
        ...
        for i in range(len(politician_list)):
            pol_info_thread = restful_pol_info.RestfulUserInfo(politician_list[i], db_name,
                                                               politician_info_collection_name, state_list[i],
                                                               ele_list[i],
                                                               party_list[i])
        ...
        logger.info('============================================')
        pol_tweets_start_date = start_date
        ...
        logger.info(
            "Harvest politician's info finishes. {} mins used ".format((time.time() - pol_info_start_time) / 60))

        # read politician info from mongoDB
        ...
        # ===Extract top five hash tags from politicians' tweets===
        ...
        # harvest user's tweets based on the top five hash tags
        ...
        logger.info(
            'Harvest tweets with hashtags finished. Time used: %f mins' % (
                        (time.time() - hashtag_tweets_start_time) / 60))

        # read mentioned tweets with state
        logger.info('============================================')
