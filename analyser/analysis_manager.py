...
import functional_tools, tweets_analysis, attach_state, cumulative_analysis, daily_analysis
...
if __name__ == '__main__':
    ...
    for i in range(len(politician_list)):
        pol_info_thread = restful_pol_info.RestfulUserInfo(politician_list[i], db_name,
                        politician_info_collection_name, state_list[i], ele_list[i],party_list[i])
    ...
    for i in range(len(politician_list)):
        pol_tweets_thread = restful_pol_tweets.RestfulPolTweets(politician_list[i], db_name,
                        politician_tweet_collection_name, state_list[i], ele_list[i], party_list[i], start_date)
    ...
    logger.info("Harvest politician's tweets finishes. {} mins used.".format((time.time() - pol_tweets_start_time) / 60))
    ...
    logger.info('{} mentioned tweets loaded. Time used: {} mins'.format(len(totalMention_df.index), (time.time() - read_men_tweets_start_time) / 60))
    ...
    cumulative_analysis = cumulative_analysis.CumulativeAnalysis(start_date, end_date, f_tools, find_ip_address,
                                                pol_tweets_df, politician_df)

    cumulative_analysis.start()

    # mentionState_from_mongo = f_tools.find_mongo_by_date('backup', 'totalMentionedWithState',
    #                                                  datetime(2019, 4, 13), end_date, find_ip_address)
    ...
    logger.info('Cumulative analysis finishes. Time used: %f mins' % ((time.time() - cumulative_analysis_start_time) / 60))

    restful_hashtag.join()
    logger.info('Harvest tweets with hashtags finished. Time used: %f mins' % ((time.time() - hashtag_tweets_start_time) / 60))
    logger.info('============================================')
    logger.info("Daily analysis starts")
    daily_analysis_start_time = time.time()
    daily_analysis = daily_analysis.DailyAnalysis(start_date, end_date, f_tools, find_ip_address,
