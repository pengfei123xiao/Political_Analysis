...
class DailyAnalysis(threading.Thread):
    ...
    """
    def __init__(self, start_date, end_date, f_tools, find_ip_address, totalMention_df, pol_tweets_df, politician_df):
        """
        """
        ...
        self.find_ip_address = find_ip_address
...
def run():
    ...
    for i in range((self.end_date-self.start_date).days-1):
        ...
        new_pol_tweets_df = self.pol_tweets_df[(self.pol_tweets_df['Date'] >= start) & (self.pol_tweets_df['Date'] < end)]

        user_hashtag_tweets_mongo = f_tools.find_mongo_by_date(db_name, 'restfulByHashtag', start, end)
        user_hashtag_tweets_df = pd.DataFrame(list(user_hashtag_tweets_mongo))
        del user_hashtag_tweets_mongo

        new_mention_df = self.totalMention_df[(self.totalMention_df['Date'] >= start) & (self.totalMention_df['Date'] < end)]
        ...
        pol_top_tags = self.f_tools.count_popular_hashtag(pol_tweets_df)
        user_top_tags = self.f_fools.count_popular_hashtag(user_hashtag_tweets_df)
        ...
        self.f_tools.save_data(result_dict, 'test', 'dailyTest', 'insert_one')
